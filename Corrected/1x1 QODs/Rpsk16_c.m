...
zpsk16 = exp((1i*(pi/8)*[0:15]));    %psk16 "Unit Power"
zpsk16r  = exp((1i*(pi/8)*[0:15]))*exp(1i*(pi/16));    %psk16 rotated "Unit Power"
...
for Case=1:2
    for n=1:length(snrdb)
        for loop=1:2:size(sym1,2)        %Two symbols at every iteration.
            ...
            w1_0 = (sqrt(snrx(n))/2)*(1/sqrt(2))*(randn(1,1) + 1i*randn(1,1));        %Noise Profile RH
            w1_1 = (sqrt(snrx(n))/2)*(1/sqrt(2))*(randn(1,1) + 1i*randn(1,1));        %Noise Profile RV
