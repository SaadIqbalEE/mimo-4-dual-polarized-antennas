    ...
    %%%%%%%% Received Signal at Decoder %%%%%%%%%%%
            ...
            for x1=1:lenz(2)
                for x2=1:lenz(2)
                    ...
                    if Case == 1
                        DR = -1*real(C1*H1*conj(RH0) + conj(C1)*RV0*conj(H2) - C2*conj(RH0)*conj(H2) + conj(C2)*RV0*H1);
                    elseif Case == 2
                        DR = -1*real(C1*H1*conj(RH0) + conj(C1)*RV0*conj(H2) -C2*exp(1i*(pi/4))*conj(RH0)*conj(H2) + conj(C2*exp(1i*(pi/4)))*RV0*H1);
                    end
