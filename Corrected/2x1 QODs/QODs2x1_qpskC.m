...
for Case=1:2
    for n=1:length(snrdb)
        for loop=1:4:size(sym1,2)        %Two symbols at every iteration.
            ...
            w1_00 = (sqrt(snrx(n))/2)*(randn(1,1) + 1i*randn(1,1));        %Noise Profile RH 0
            w1_10 = (sqrt(snrx(n))/2)*(randn(1,1) + 1i*randn(1,1));        %Noise Profile RV 0
            w1_01 = (sqrt(snrx(n))/2)*(randn(1,1) + 1i*randn(1,1));        %Noise Profile RH 1
            w1_11 = (sqrt(snrx(n))/2)*(randn(1,1) + 1i*randn(1,1));        %Noise Profile RV 1
            ...
            elseif (Case == 2)
                RH0 = q1*H1 + q4*H3 - phase*(q2*conj(H2) + q3*conj(H4)) + w1_00;      %Received at RH0            
                RV0 = q1*H2 + q4*H4 + phase*(q2*conj(H1) + q3*conj(H3)) + w1_10;      %Received at RV0
                RH1 = conj(q2)*H1 - conj(q3)*H3 + conj(phase)*(conj(q1)*conj(H2) - conj(q4)*conj(H4)) + w1_01;      %Received at RH1            
                RV1 = conj(q2)*H2 - conj(q3)*H4 + conj(phase)*(-conj(q1)*conj(H1) + conj(q4)*conj(H3)) + w1_11;      %Received at RV1
    ...
    %%%%%%%% Received Signal at Decoder %%%%%%%%%%%
            ...
            for lx=1:lenz(2)
                ...
                elseif Case == 2
                    DR1 = -1*real(conj(RH0)*C*H1 + conj(RV0)*C*H2 + conj(phase)*conj(C)*(conj(RH1)*conj(H2) - conj(RV1)*conj(H1)));       %Decoding first Symbol
                    ...
                    DR4 = -1*real(conj(RH0)*C*H3 + conj(RV0)*C*H4 + conj(phase)*conj(C)*(-conj(RH1)*conj(H4) + conj(RV1)*conj(H3)));       %Decoding fourth Symbol
                end
